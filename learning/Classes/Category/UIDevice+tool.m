...
+ (BOOL)isJailBreak{
    NSArray *jailBreakPaths = [self jailBreakPaths];
    for (NSString *obj in jailBreakPaths) {
...
+ (NSString *)wifiName{
    ...
    if (!wifiInfo) return nil;
    return [wifiInfo valueForKey:@"SSID"];
}

+ (NSString *)wifiMac{
    NSDictionary *wifiInfo = [self wifiInfo];
    if (!wifiInfo) return nil;
    return [wifiInfo valueForKey:@"BSSID"];
}
